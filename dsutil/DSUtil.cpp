...
BOOL IsRenderer(IBaseFilter* aFilter)
{
	if (!aFilter)
	{
		return FALSE;
	...
	if(CComQIPtr<IAMFilterMiscFlags> pAMMF = aFilter)
	{
	...
	{
		BeginEnumPins(aFilter, spEnum, spPin)
		{
			CComPtr<IPin> spPinIn;
			DWORD size = 1;
			if(SUCCEEDED(spPin->QueryInternalConnections(&spPinIn, &size)) && size == 0)
			{
				return TRUE;
				break;
			}
...
GUID GUIDFromCString(LPCTSTR str)
{
	GUID guid = GUID_NULL;
...
HRESULT GUIDFromCString(LPCTSTR str, GUID& guid)
{
	guid = GUID_NULL;
...
static struct {LPCSTR name, iso6392, iso6391;} s_isolangs[] = 
{
	...
	{"Bini", "bin", ""},
	{"Bislama", "bis", "bi"},
	{"Bokm表, Norwegian; Norwegian Bokm表", "nob", "nb"},
	{"Bosnian", "bos", "bs"},
	{"Braj", "bra", ""},
	{"Breton", "bre", "br"},
	{"Buginese", "bug", ""},
	...
	{"Norwegian", "nor", "no"},
	{"Norwegian Bokm表; Bokm表, Norwegian", "nob", "nb"},
	...
	{"Nyoro", "nyo", ""},
	{"Nzima", "nzi", ""},
	{"Occitan (post 1500},; Proven鏰l", "oci", "oc"},
	{"Ojibwa", "oji", "oj"},
